...
│   （statistics.js / finance.js 尚未建立）
├── tools/
│   └── export-md.mjs   # 把教材與題庫匯出成 Markdown（給第三方檢查用）+ 品質檢核
...
合計 **100 個知識點、388 張名詞卡、300 題選擇、100 題申論、109 題口試**（含 9 題通用面試題）、5 間學校側重。

**程式面功能已完整，瓶頸完全在內容。** 行銷、管理、經濟三科十章全數完成；統計與財管尚未開檔（連 `content/statistics.js` 這類檔案都還沒有），且**這兩科開工前要先解決公式排版與計算題結構**（見 P2）。
...
**行銷、管理、經濟三科的十章已於 2026-08-10 全數補完。** 剩下統計學與財務管理，但**這兩科不能直接開工**——先解決下面兩件事：

1. **計算題的資料結構**：目前 MCQ 沒有「顯示計算過程」的欄位。要決定是 `explain` 直接寫算式，還是另加 `steps[]`。
2. **公式排版的極限**：行內公式 `` `x` `` 已支援（等寬字體，見「內容模型」規則 4），但**分數、上下標、根號、Σ 目前只能用文字近似**（`ΔQ/Q`、`Q^d`）。統計的機率分配與財管的現值公式會不會超過可讀範圍，需要先做一章試寫再決定。**不要引入 KaTeX**——會破壞零依賴與離線。

**建議先開統計學**（研究方法是台大商研的追問重點，且與行銷第 4 章行銷研究直接互補）。
建議章節：敘述統計 → 機率 → 抽樣分配 → 估計 → 假設檢定 → 變異數分析 → 相關與迴歸 → 類別資料分析 → 無母數 → 研究設計與常見誤用。
...
它同時會把教材與題庫輸出到 `export/`（六個 Markdown 檔，可直接丟給 NotebookLM 之類的工具做第三方內容檢查；`export/` 已在 `.gitignore`）。檢核項目：選擇題格式、詳解是否提到選項代號、知識點欄位完整性、id 重複、表格語法、**跨章連結是否指向不存在的知識點**、名詞卡英文、知識點出處、範答長度。**全部應為「無」。**
...
- [ ] **公式排版只解了一半**。2026-08-10 補上**行內公式** `` `x` `` 的渲染（等寬字體＋淡底，`ui.js` 的 `inline()`），經濟學整科已在使用，`Ed = (ΔQ/Q) ÷ (ΔP/P)` 這類單行算式可讀性良好。**尚未解決的是分數、上下標、根號與 Σ**——目前只能用文字近似（`ΔQ/Q`、`Q^d`）。統計的機率分配與財管的現值公式是否仍在可讀範圍內，需要先試寫一章再決定。**不要引入 KaTeX**——會破壞零依賴與離線。
