...
合計 **114 個知識點、444 張名詞卡、342 題選擇、114 題申論、124 題口試**（含 9 題通用面試題）、5 間學校側重。
...
**行銷、管理、經濟三科的十章已於 2026-08-10 全數補完；統計學同日開檔並完成第 1–3 章。**

**下一步就是照順序補統計學第 4–10 章**（章節 id 已在 `statistics.js` 檔頭預留）：
估計 → 假設檢定 → 變異數分析 → 相關與迴歸 → 類別資料分析 → 無母數方法 → 研究設計與常見誤用。
