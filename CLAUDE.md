...
合計 **128 個知識點、500 張名詞卡、384 題選擇、128 題申論、138 題口試**（含 9 題通用面試題）、5 間學校側重。
...
**行銷、管理、經濟三科的十章已於 2026-08-10 全數補完；統計學同日開檔並完成第 1–7 章（25 個知識點）。**

**下一步是統計學第 8–10 章**（章節 id 已在 `statistics.js` 檔頭預留）：
stat-08 類別資料分析（卡方適合度／獨立性、列聯表）→ stat-09 無母數方法（Mann–Whitney、Wilcoxon、Kruskal–Wallis）→ stat-10 研究設計與常見誤用。
之後才是財務管理整科（**財管的現值公式一樣建議先試寫一章再決定排版**）。

**第 1–7 章裡有 3 條指向第 9 章的純文字參照**（無母數方法），寫完 stat-09 後記得回頭把它們改成 `[[stat-09-xx]]` 連結——grep `第 9 章` 就找得到。這是下方「分批寫一科的兩個坑」第 1 點的既定作法。
